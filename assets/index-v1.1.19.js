...
async function ni(S) {
  ...
  return d.map((x) => ({
    url: `${j.host}${x.path}/256/6/${S.lat}/${S.lon}/2/1_1.png`,
...
async function b0(S, M) {
  const j = `${S.host}${S.path}/256/6/${M.lat}/${M.lon}/2/1_1.png`,
...
async function Ld(S, M) {
  ...
  try {
    const j = await Promise.all(S.map((jl) => b0(jl, M))),
      ...
      d = [];
    if (lastIndex >= 0) {
      let tracked = j[lastIndex][0];
      d.unshift({ sample: tracked, index: lastIndex });
      for (let index = lastIndex - 1; index >= 0; index--) {
        ...
        if (match && Math.hypot(match.x - tracked.x, match.y - tracked.y) <= 42)
          ((tracked = match), d.unshift({ sample: tracked, index }));
      }
    }
    if (!d.length)
      return {
  ...
  return (
    w.useEffect(() => {
      ...
      return (
        (async () => {
          const N = await _f(
            () => import("./leaflet-v1.1.19.js").then((D) => D.l),
            [],
            import.meta.url,
          );
          ...
          ((d.current = N.map(j.current, {
            ...
            M &&
              (x.current = N.tileLayer(M.tileUrl, {
                opacity: 0.88,
                zIndex: 5,
    ...
    w.useEffect(() => {
      !d.current ||
        ...
        (async () => {
          const C = await _f(
            () => import("./leaflet-v1.1.19.js").then((N) => N.l),
            [],
            import.meta.url,
          );
          (x.current && d.current.removeLayer(x.current),
            (x.current = C.tileLayer(M.tileUrl, {
              opacity: 0.88,
              zIndex: 5,
              className: "radarTiles",
              maxNativeZoom: 6,
  ...
  return (
    ...
    w.useEffect(() => {
      ...
      return (
        (async () => {
          const N = await _f(
            () => import("./leaflet-v1.1.19.js").then((O) => O.l),
            [],
            import.meta.url,
          );
        ...
        children: [
          ...
          f.jsxs("div", {
            children: [
              ...
              f.jsx("small", { children: "APP METEO LOCALE · v1.1.19" }),
            ],
          }),
          f.jsx("button", {
          ...
          children: [
            ...
            f.jsxs("div", {
              children: [
                ...
                f.jsx("strong", { children: "Grandine Alert 1.1.19" }),
                f.jsx("p", {
                  children:
                    "Aggiunte impostazioni notifiche, preferenze locali e pulsante per provare il permesso sul dispositivo.",
                }),
              ],
            }),
            f.jsx("button", {
        ...
        children: [
          f.jsxs("div", {
            ...
            children: [
              ...
              f.jsx("span", { children: "Versione 1.1.19" }),
