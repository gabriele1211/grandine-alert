  ...
  return (
    w.useEffect(() => {
      ...
      return (
        (async () => {
          const N = await _f(
            () => import("./leaflet-v1.1.26.js").then((D) => D.l),
            [],
            import.meta.url,
          );
    ...
    w.useEffect(() => {
      !d.current ||
        ...
        (async () => {
          const C = await _f(
            () => import("./leaflet-v1.1.26.js").then((N) => N.l),
            [],
            import.meta.url,
          );
  ...
  return (
    ...
    w.useEffect(() => {
      ...
      return (
        (async () => {
          const N = await _f(
            () => import("./leaflet-v1.1.26.js").then((O) => O.l),
            [],
            import.meta.url,
          );
      ...
      const bl = () => {
        ...
        navigator.geolocation.getCurrentPosition(
          async (Ol) => {
            if (Y) return;
            const fa = {
              lat: Ol.coords.latitude,
              lon: Ol.coords.longitude,
            };
            ((zt.current = fa),
              d(fa),
              localStorage.setItem(
              ...
              Tl("Posizione GPS aggiornata: controllo la nuova zona…"));
            const mt = await Promise.allSettled([Pa(fa), ni(fa)]);
            if (Y) return;
            (mt[0].status === "fulfilled" && Z(mt[0].value),
              mt[1].status === "fulfilled"
                ? (R(mt[1].value), al(0), I(!1))
                : I(!0),
              (Qt.current = fa),
              (ca.current = Date.now()),
              Vl(Date.now() + 6e5),
              rt(600),
              Tl(
                `Posizione aggiornata alle ${new Date().toLocaleTimeString("it-IT", { hour: "2-digit", minute: "2-digit" })}.`,
              ),
              (Q = !1));
          },
          () => {
            ((Q = !1),
              Tl(
                "Posizione GPS non aggiornata: controllo permesso e segnale.",
              ));
          },
          { enableHighAccuracy: !0, maximumAge: 0, timeout: 2e4 },
        );
      };
      return (
        bl(),
        document.addEventListener("visibilitychange", bl),
        window.addEventListener("focus", bl),
        () => {
          ((Y = !0),
            document.removeEventListener("visibilitychange", bl),
            window.removeEventListener("focus", bl));
        }
      );
    }, []),
    w.useEffect(() => {
        ...
        Q <= 0
          ? (rt(600),
            ...
            navigator.geolocation.getCurrentPosition(
              async (bl) => {
                const Ol = {
                  lat: bl.coords.latitude,
                  lon: bl.coords.longitude,
                };
                ((zt.current = Ol),
                  d(Ol),
                  localStorage.setItem(
                ...
                const fa = await Promise.allSettled([Pa(Ol), ni(Ol)]);
                (fa[0].status === "fulfilled" && Z(fa[0].value),
                  fa[1].status === "fulfilled"
                    ? (R(fa[1].value), al(0), I(!1))
                    : I(!0),
                  (Qt.current = Ol),
                  (ca.current = Date.now()),
                  x &&
                    Pa(x)
                      ...
                      .catch(() => {}),
                  Tl(
                    `Posizione GPS aggiornata automaticamente alle ${new Date().toLocaleTimeString("it-IT", { hour: "2-digit", minute: "2-digit" })}.`,
                  ),
                  Vl(Date.now() + 6e5));
              },
              () => {
                (Tl(
                  "GPS non disponibile: mantengo l’ultima posizione e riprovo tra 10 minuti.",
                ),
                  Pa(j)
                    .then(Z)
                    .catch(() => {}),
        ...
        children: [
          ...
          f.jsxs("div", {
            children: [
              ...
              f.jsx("small", { children: "APP METEO LOCALE · v1.1.26" }),
            ],
          }),
          f.jsx("button", {
          ...
          children: [
            ...
            f.jsxs("div", {
              children: [
                ...
                f.jsx("strong", { children: "Grandine Alert 1.1.26" }),
                f.jsx("p", {
                  children:
        ...
        children: [
          f.jsxs("div", {
            ...
            children: [
              ...
              f.jsx("span", { children: "Versione 1.1.26" }),
